...
class ReadingSiteTest(unittest.TestCase):
    ...
    def test_builds_index_and_article_pages_with_keywords_summary_and_source(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("导出本周 Markdown", index)
            ...
            self.assertIn("不止收藏。", index)
            self.assertIn("可信正文进入译文库，受限与污染来源进入恢复队列。", index)
            ...
            self.assertIn('class="feature-stage reveal"', index)
            self.assertIn('class="recovery-section reveal"', index)
            self.assertEqual(index.count('<article class="article-card editorial-card'), 2)
            ...
            self.assertIn(".library-hero { position: relative;", style)
            self.assertIn(".hero-lede", style)

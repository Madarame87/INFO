...
def prepare_workdir(article, resolved):
...
def call_pi(workdir):
    prompt = (
        ...
        f"Workdir: {workdir}\n\n"
        "Read source.md and meta.json from that directory. If source.md is absent and "
        "meta.json says resolutionStatus=needs_browser_resolution, use the skill's Chrome "
        "Transcript UI fallback. Write only the result JSON to meta.json.resultFile."
    )
    try:
        proc = subprocess.run(
            ["pi", "-p", prompt],
...
def process_article(article):
    workdir = workdir_for(article)
    try:
        resolved = resolve_article(article, workdir)
        workdir, meta = prepare_workdir(article, resolved)
        log(f"🚀 启动 pi -p: {article.get('title') or article['url']}")
        call_pi(workdir)
        result = validate_result(meta["resultFile"])
        return {
            "url": article["url"],
            "status": "done",
            "processedAt": utc_iso(),
            "meta": done_meta(result, meta),
        }

...
def api_base_url_error(cfg):
    ...
    if not base_url:
        return "模型 API baseUrl 未配置"
    try:
        parsed = urlparse(base_url)
    except ValueError:
        return "模型 API baseUrl 格式无效"
    if not parsed.hostname or parsed.username or parsed.password:
        return "模型 API baseUrl 不得包含凭据，且必须包含主机名"
    if parsed.query or parsed.fragment:
        return "模型 API baseUrl 不得包含 query 或 fragment"
    if parsed.scheme not in {"https", "http"}:
        return "模型 API baseUrl 必须使用 HTTPS"
    ...
    if parsed.scheme != "https" and not allow_insecure_test:
        return "自定义模型 API baseUrl 必须使用 HTTPS"
    if not allow_custom and not allow_insecure_test:
        return "自定义模型 API baseUrl 需要显式设置 INFO_COLLECTOR_ALLOW_CUSTOM_API_BASE=1"
    return None
